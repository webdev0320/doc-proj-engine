...
def run_pipeline(blob_id: str, pdf_path: str, storage_settings: Optional[Dict[str, Any]] = None):
    try:
        ...
        for i in range(len(doc)):
            ...
            if storage_settings:
                def _upload(ip, rip, ss):
                    try:
                        upload_to_remote(ip, rip, ss)
                    except Exception as ue:
                        print(f"!!! [PIPELINE ERROR] Page upload failed: {ue}")
                import threading
                remote_image_path = f"pages/{image_filename}"
                print(f">>> [PIPELINE] Uploading page {page_num} to SFTP: {remote_image_path} (async)")
                threading.Thread(target=_upload, args=(image_path, remote_image_path, storage_settings)).start()
        ...
        try:
            ...
            for i in range(0, len(page_records), chunk_size):
                ...
                r = requests.post(f"{BACKEND_URL}/api/blobs/{blob_id}/pages", json=payload, timeout=30)
                print(f">>> [CALLBACK] POST /api/blobs/{blob_id}/pages (chunk {i//chunk_size + 1}) -> {r.status_code} {r.text}")
                r.raise_for_status()
    ...
    try:
        ...
        for i in range(len(doc)):
            ...
            if storage_settings:
                ...
                import threading
                remote_image_path = f"pages/{image_filename}"
                print(f">>> [APPEND] Uploading page {page_num} to SFTP: {remote_image_path} (async)")
                threading.Thread(target=_upload_append, args=(image_path, remote_image_path, storage_settings)).start()
        ...
        try:
            ...
            for i in range(0, len(page_records), chunk_size):
                ...
                r = requests.post(
                    ...
                    json={"pages": chunk, "mode": "append"},
                    timeout=30
                )
                print(f">>> [CALLBACK] POST /api/blobs/{blob_id}/pages (append chunk {i//chunk_size + 1}) -> {r.status_code} {r.text}")
                r.raise_for_status()
        except Exception as cb_e:
